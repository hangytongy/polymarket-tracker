            ...
            if token_id:
                ...
                if current_orders:
                    ...
                    if buy_orders:
                        ...
                        if min_buy_price >= reward_bid_min and max_buy_price <= reward_mid_range:
                            ...
                            if resp:
                                ...
                                message = f"{str(resp)} \n\nOrder out of reward range, cancel old and placed order at {bid_price} to {reward_bid_min} for {question} and {side} and size {size}"
                                send_telegram_message(message)
                            else:
                    ...
                    if resp:
                        print(f"placed order at {bid_price} for {question} and {side}")
                        message = f"{str(resp)} \n\nPlaced order at {bid_price} to {reward_bid_min} for {question} and {side} and size {size}"
                        send_telegram_message(message)
                    else:
                        message = f"Error placing scale order -> either steps is 0 or size per order less than reward size"
                        send_telegram_message(message)
