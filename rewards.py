...
try:
    ...
    for _, row in df.iterrows():
        ...
        if token_id:
            ...
            mid_range = (reward_bid_max + reward_bid_min) / 2
            mid_range = round(mid_range,decimal_places)
            mid_range_lower = round((mid_range + reward_bid_min) / 2,decimal_places)
            mid_range_upper = round((mid_range + reward_bid_max) / 2,decimal_places)

            print(f"Mid range: {mid_range}, Mid range lower: {mid_range_lower}, Mid range upper: {mid_range_upper}")
